...
// 必须在类内部显式声明 extern，编译器才知道这是类的一个成员方法
...
function UNI_cell NNI_cell::to_UNI(input bit [7:0] uni_VPI);
   ...
   copy.GFC     = 4'b0000;         // 硬件自动清零并垫上 4 位 GFC
   copy.VPI     = uni_VPI;         // 填入转发表配置的新 VPI
   copy.VCI     = this.VCI;        // this 现在合法了，代表当前 NNI 信元自己
   copy.CLP     = this.CLP; 
   copy.PT      = this.PT;  
   copy.Payload = this.Payload;    // 载荷数据完美透传
   
   // 重新利用 UNI 类的 hec 算法计算输出端符合 UNI 规范的全新校验和
   copy.HEC     = copy.hec({4'b0000, copy.VPI, copy.VCI, copy.CLP, copy.PT});
